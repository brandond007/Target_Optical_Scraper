...
if [ -t 0 ]; then
  read -rp "Press Enter to exit setup.sh..."
fi

# Drop user into the venv shell if running interactively (for ./setup.sh)
...
# Otherwise just deactivate and exit (for source setup.sh)
deactivate
exit 0
